...
- **Inspirações Tess AI, versão-cérebro (análise 2026-07-09):** (a) **⭐ Duelo de Modelos** — mesma peça em 2–3 providers lado a lado + voto do vencedor; barato (Studio já multi-provider) e gera PREFERÊNCIA PAREADA (ouro p/ win-rate do cérebro e p/ dataset de fine-tune); (b) **Autopilot on-brand** — agente gera→avalia com o cérebro-juiz (`check_on_brand`)→refina em background→entrega top-3 com parecer; (c) **Brand Deck** — apresentação da marca (identidade+território+aprendizados) em PPTX 1-clique (pptxgenjs já é dep; ≠ do export do painel vetado — é entregável do cliente); (d) **contexto de campanha persistente** reutilizável entre Writing/Studio/Workflow. Narrativa sem código: "usuários ilimitados — pague pelo que cria, não por cadeira" (créditos ≠ assentos).
