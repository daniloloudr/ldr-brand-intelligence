...
- **🚀 MCP do Cérebro — EM EXECUÇÃO (decisão 2026-07-08: Figma primeiro).** Plano completo em [`specs/features/mcp-cerebro.md`](specs/features/mcp-cerebro.md): F0 auth por API key (migration 035) → F1 MCP server no Netlify (6 tools sobre o `_brain.js`) → F2 teste estruturado T1–T7 com o MCP oficial do Figma (write-to-canvas) → F3 produto. Dependência do Danilo: seat Full pago no Figma p/ o T4.
