...
- **Resiliência (jul/2026):** a geração tenta **até 2 vezes** (falha de API/timeout ou JSON inválido; backoff 4s) antes de gravar `status='error'`. Um **reaper** agendado (`diagnostico-reaper.js`, cron `*/15`) marca como `error` qualquer diagnóstico preso em `running` há mais de **15 min** — cobre jobs órfãos (background interrompida: dev server morto, timeout duro, crash sem catch), que antes ficavam "em andamento" pra sempre.
- **Timeout da chamada de IA (bug prod × localhost):** a chamada não-streaming com `web_search` **enrola por minutos em prod** (web search lento na Lambda; no localhost/`netlify dev` não há teto, então completava — em prod estourava os 15 min da background function e o reaper marcava `error`). Fix em `_ai.js`: `callAI` ganhou **`timeoutMs`** (AbortController cobrindo fetch + leitura do corpo → converte hang em `AIError 408` retentável) e o tool `web_search` ganhou **`max_uses: 6`** (bounda o loop de busca; o prompt pede 5). Diagnóstico usa **`timeoutMs: 390000` (6,5 min) × 2 tentativas** (~13 min, cabe nos 15) — poucas tentativas com tempo real de completar batem "muitas tentativas curtas que abortam uma geração lenta que ia dar certo".
