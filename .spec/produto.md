...
**Changelog v8.0 (20–21/jul/2026) — GO-LIVE DO 1º CLIENTE (Hering): identidade s1ngulr, multitenant por subdomínio, cobrança por-workspace, URLs limpas e onboarding automático.**

**IDENTIDADE s1ngulr = estilo Vercel light (decisão Danilo — distinta da LOUDR):** monocromático (branco/preto/cinza), **fundo branco sempre** (Danilo não quer app com fundo preto). Reskin **só nos tokens**: `theme.js` themeLight (primary teal→**preto**, bg→branco, texto/bordas neutros) + `constants.js` `DS` (navy/text/border→monocromático) — as **cores semânticas** (verde/rosa/âmbar de scores, sentimento, gráficos) preservadas, como o Vercel faz (UI monocromática + dados coloridos). **Login refeito** (CSS puro): split estilo Runway (showcase à esquerda com textura geométrica + a proposta "A marca no meio da operação"; form Vercel-light à direita, botão preto, wordmark `s1ngulr`). Migração do app pra **Tailwind/shadcn** fica pra quando a marca definitiva estiver pronta (decisão deliberada — 63 telas em MUI). Follow-ups: hexes hardcoded + botões do admin (`DS.green`) ainda teal.

**MULTITENANT POR SUBDOMÍNIO — LIVE (reabre a decisão de 15/07; gatilho = 1º cliente externo):** `nomedamarca.s1ngulr.com`. `s1ngulr.com` (registrado na GoDaddy) migrado pro **Netlify DNS** (nameservers nsone). Resolução por hostname client-side (`getTenantSlug` em helpers; **RLS por workspace_id segue o perímetro real**; sessão isolada por subdomínio; `app.s1ngulr.com` = login/admin). **Auto-provisionamento:** wildcard não é self-serve no Netlify (UI e API rejeitam), então `admin-create-workspace` registra `{slug}.s1ngulr.com` como domain alias **via API do Netlify** na criação (DNS + cert automáticos) — requer `NETLIFY_API_TOKEN` no env. `workspaces.slug` (migration 044).
