...
| ~~**Backup do banco**~~ 🔧 CÓDIGO PRONTO 2026-07-15 | dump diário (GitHub Actions → R2) + dump pré-migration (`scripts/migrate.sh`) + doc de restore ([`backup.md`](backup.md)). PITR adiado (plano Pro). **Pendente Danilo:** criar bucket R2 `s1ngulr-backups` + secrets do GitHub (SUPABASE_DB_URL session-pooler + R2_*) + pg_dump/aws-cli local. Regra nova: `db push` só via migrate.sh (backup antes) | ✅ código · setup c/ Danilo |
