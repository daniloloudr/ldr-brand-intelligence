...
| **S1 🟡 1 de 7 feito — o resto é decisão do Danilo** | **Duas identidades para a mesma pessoa** — conta de operação ≠ conta de super admin (só `platform_admins`, **nunca** membro de workspace).<br><br>✅ **BR4NDCODE, 30/ago.** O Danilo saiu de `workspace_members`; o Leo segue owner, então o workspace continua governável e o trigger `protege_ultimo_owner` não foi acionado. **Medido depois:** existem 22 chunks de brand book, 8 sinais, 1 brand book, 1 versão de cérebro, 1 marca e 1 diagnóstico — e ele enxerga **0 de cada um** sem sessão aberta. O `/admin` seguiu inteiro (9 workspaces, 124 leads). **É o primeiro tenant onde a separação vale de verdade.**<br><br>🔴 **Os outros seis esperam decisão.** "Zero código" não é "zero decisão": o Danilo é o **único owner** de Hering, LOUDR, PES English, Pixel Retail, scolex e Worten. Tirá-lo dispara o trigger — e, se não disparasse, deixaria seis workspaces **sem dono**, que é o problema da Zétona e da Escola que acabamos de fechar. **Falta nomear um dono por tenant:** Hering → Rafael Passos? · Pixel → Marcio? · Worten → o Zé, quando o deal fechar? · PES/scolex → ? · **LOUDR é a nossa própria empresa**, e vale decidir se o workspace da casa entra na regra. Mesma forma do S0: item marcado como fácil com pré-requisito escondido | 🔴 **decisão por tenant** |
...
| ~~**S4**~~ ✅ **27/ago · tela 31/ago** | **Trilha de auditoria** — nasceu junto, como previsto: a linha de `platform_admin_sessions` É o registro (quem, qual tenant, quando, por quanto tempo, por quê). `motivo` é `not null` com CHECK de 3 caracteres, e o endpoint recusa antes de gravar. ~~Ainda não tem tela~~ ✅ **31/ago (v8.6): `/admin` → Acessos.** Tenant, motivo, início, duração, origem e estado, com alerta para sessões abertas — e a distinção **encerrada** (fechada de propósito) × **vencida** (morreu no prazo), que é a garantia real. Foi frontend puro: a 053 já tinha criado a policy de select e o `grant`. ⚠️ `admin_user_id = auth.uid()` — cada operador lê as PRÓPRIAS sessões; multi-operador exige policy nova ou leitura pelo servidor | ✅ |
...
*F4 — os 7 ajustes da reunião (31/ago/2026):* triados contra o código no mesmo dia, **e revisados com o Danilo na sequência — a revisão mudou dois dos sete.** A leitura final: 2 são o mesmo defeito visto de dois ângulos, 1 era diagnóstico errado meu e virou o conserto mais barato da lista, 1 fica parado por decisão, 1 pode estar barrado por regulação antes de ser código. Detalhe técnico em [`features/piloto-hering.md`](features/piloto-hering.md) § F4.

| # | Pedido | O que se achou no código | Tamanho |
|---|---|---|---|
...
**Fechado junto, na mesma release:** teto de referências **5 → 10** (piso dos grandes, levantado na doc do fal) · a página **Imagem** alinhada ao Fluxo (mesmo teto + aviso de como o modelo lê) · o nó **Vídeo** no painel (usa só a 1ª imagem) · **atalhos** Cmd/Ctrl+S, Z e Shift+Z · **editor grande** no Prompt e Contexto · **base de casting** como template · o **"Melhorar prompt"** virou revisor (300 caracteres, Haiku, proibido acrescentar) · **`/admin` → Acessos**, a trilha da 053 com tela · **R8 conferido** (alfa real, 4K, e o Recortar preserva transparência).

### O `MAX_REF` é nosso — a resposta curta

**É nosso, e é inventado.** `MAX_REF = 5` é uma constante nossa em `studioNodes.jsx:513`, aplicada igual para **todo** modelo. Não existe em lugar nenhum do código um teto por modelo: o catálogo (`src/lib/studioModels.js`) só sabe `refs: true|false` — se aceita referência, não quantas.

Ou seja: hoje o produto impõe o mesmo 5 ao Seedream, ao Nano Banana e ao GPT Image 2, que têm tetos **diferentes e maiores**. O número não veio de medição nem de documentação — veio de um chute que virou constante.

⚠️ **Mas subir a constante não é o conserto.** Os tetos reais precisam ser conferidos na doc do fal, modelo a modelo, e virar dado do catálogo (`maxRefs` por linha) — senão a gente troca truncamento silencioso por 422 do provedor. E, com teto certo ou errado, **o excedente tem que virar aviso no nó**: o defeito que mordeu a Hering não foi o número 5, foi o silêncio.
...
- [ ] **Manual da marca (PDF) — área própria** 🟡 — marcas que JÁ têm manual: (a) upload do PDF salvo no storage + visualização embutida (a "casa" do manual dentro do produto, provável aba no Brand Kit dos Ativos ou na Expressão); (b) a raspagem de texto já existe (`brand-manual-extract-background`; **F11 fechado em 26/08** — já grava em `strategy`); (c) **NOVO: o VISUAL do manual popula o cérebro** — páginas renderizadas como imagens viram referências visuais (alimentam `reference_upload` + hints de geração). O manual ensina pelo texto E pela estética.
