...
| 3 🟢→✅ 14/jul (parcial) | **Observabilidade zero** | **Watchdog ENTREGUE:** `cron_runs`/`cron_alerts` (migration 041) + `withHeartbeat()` nos 6 scheduled + `cron-watchdog` horário (silêncio/morte/erro, dedup 24h, período de graça). Alerta via Netlify logs + tabela + Sentry (store API, sem SDK) + webhook. **Falta:** Danilo criar conta Sentry → setar `SENTRY_DSN` no env do Netlify (opcional: `ALERT_WEBHOOK_URL`) | Gap 1 do H1 | antes da 3ª marca externa |
...
| ~~**Gap 1 — Observabilidade**~~ ✅ 14/jul (watchdog) | Watchdog de crons entregue (migration 041 + `_watchdog.js` + `cron-watchdog` horário — heartbeat nos 6 scheduled, alerta silêncio/morte/erro com dedup e graça). Pendente só o plug do Sentry: conta do Danilo → `SENTRY_DSN` no env. Nota: o antigo `cron-monitor.js` NÃO monitora crons — é o cron de diagnóstico semanal (nome herdado) | 🟢 feito · DSN pendente |
