...
| ~~**S2**~~ ✅ **24/ago** | **MFA (TOTP) no operador** — TOTP habilitado no projeto pelo Danilo, **mais o que o console não entrega**: `MfaGate` (o `/admin` não monta sem aal2; inscrição por QR na primeira vez) e `_mfa.js` (os 5 endpoints de operador conferem a claim `aal` do token, depois de validar a assinatura). Gate de tela não protege contra token roubado — quem tem o token chama a function direto. **Falta o Danilo inscrever o fator.** ⚠️ *"Limit duration of AAL1 sessions" está ON no console: depois da primeira inscrição, confirmar que não derruba usuário SEM fator — senão todo cliente cai a cada 15 min* | ✅ |
