...
| **S1 🔴 TEM PRÉ-REQUISITO — decisão do Danilo** | **Duas identidades para a mesma pessoa** — conta de operação ≠ conta de super admin (só `platform_admins`, **nunca** membro de workspace). A 053 está no ar e a impersonação foi conferida ao vivo (30/ago), então o caminho técnico está livre. **Mas "zero código" não é "zero decisão":** medido em 30/ago, o Danilo é o **único owner** de Hering, LOUDR, PES English, Pixel Retail, scolex e Worten. Tirá-lo dispara o trigger `protege_ultimo_owner` da 052 — e, se não disparasse, deixaria seis workspaces **sem dono**, que é exatamente o problema da Zétona e da Escola que acabamos de fechar. **Falta nomear um dono por tenant** (Hering → Rafael Passos? · Pixel → Marcio? · Worten → o Zé, quando o deal fechar? · PES/scolex → ?). **BR4NDCODE já tem dois owners** (Danilo + Leo), então dessa ele pode sair hoje. **LOUDR é a nossa própria empresa** — provavelmente ele fica, e vale decidir se o workspace da casa entra na regra. Mesma forma do S0: item marcado como fácil que tinha pré-requisito escondido | 🟢 código · 🔴 **decisão por tenant** |
